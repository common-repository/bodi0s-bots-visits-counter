...
?>
<style type="text/css">
.geo-info {position: absolute; background: #fff; border:1px solid #ccc;width:20%; cursor:pointer;z-index:10000;box-shadow: 0px 3px 5px rgba(0, 0, 0, 0.196) !important}
...
<div class="wrap">
  ...
  <p class="submitbox"><a href="?page=<?php echo $_GET['page']; ?>&amp;bot-counter=reset&amp;_wpnonce=<?php echo wp_create_nonce( 'bot-nonce' ) ?>" class="submitdelete button-primary button-large">
...
var request = $.ajax({
	url: "<?php echo plugin_dir_url( __FILE__ ) ?>bodi0-bot-ajax.php?t=<?php echo time();?>",
    type: "GET",
	global: false,
	cache: false,
  ...
  <table style="width: 50% !important" class="allignleft">
    <tr>
      <td><?php _e("Filter (all)", "bodi0-bot-counter"); ?>:
        ...
        <a class="button-secondary button-large" href="javascript:void(0)" onclick="document.getElementById('filter').value=''; filter2(document.getElementById('filter'), 'bot-table')">
        <?php _e("Reset","bodi0-bot-counter"); ?>
        </a></td>
    </tr>
  </table>
  <table class="widefat" style="min-width:740px !important; max-width:900px !important" id="bot-table">
    <thead>
      <tr>
...
foreach($results as $result) {
	...
	echo '<td><div onclick="$(this).hide()" class="geo-info" style="display:none" id="toggle'.$result['id'].'"><div class="inner" id="content'.$result['id'].'"></div><div class="alignright" style="position:absolute;right:10px;top:10px;">&#10006;</div>
	</div>';
?>
    <div id="edit-<?php echo $result['id'] ?>" style="display:none;" class="edit">
      <form method="post" name="editform" action="?page=<?php echo $_GET['page'] ?>">
        ...
        <input type="text" name="bot-name" id="bot-name" value="<?php echo $result['bot_name']?>" maxlength="40"/>
        (<?php _e("up to 40 characters","bodi0-bot-counter"); ?>)
...
?>
      <table style="margin:0;padding:0; width:640px">
        <tr>
          <td style="width:16%"><p>
              <?php _e("Bot name","bodi0-bot-counter"); ?>:</p></td>
          <td><p>
              <input type="text" name="bot-name" id="bot-name" value="" maxlength="40"/>
              (<?php _e("up to 40 characters, required","bodi0-bot-counter"); ?>)</p></td>
        </tr>
        <tr>
          <td><p>
              <?php _e("Bot identifier","bodi0-bot-counter"); ?>:</p></td>
          <td><p>
              <input type="text" name="bot-mark" id="bot-mark" value="" maxlength="100"/>
              (<?php _e("up to 100 characters, required","bodi0-bot-counter"); ?>)</p></td>
        </tr>
        <tr>
          <td colspan="2"><p>
...
<?php _e("Statscrop: The higher the ranking is (5 maximum), the more popular the website is", "bodi0-bot-counter"); ?>. 
        ...
        <tr>
          <td style="width:10%">
              ...
              <p>
              <?php _e("URL","bodi0-bot-counter"); ?>:</p></td>
          <td><p>
